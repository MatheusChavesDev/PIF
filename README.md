# 📚 CESAR_ADS20262_2A | Programação Imperativa e Funcional

![C](https://img.shields.io/badge/c-%2300599C.svg?style=for-the-badge&logo=c&logoColor=white)
![Haskell](https://img.shields.io/badge/Haskell-5e5086?style=for-the-badge&logo=haskell&logoColor=white)
![CESAR School](https://img.shields.io/badge/CESAR_School-2026.2-FF6600?style=for-the-badge)

Repositório oficial da disciplina de **Programação Imperativa e Funcional** do curso Tecnológico em Análise e Desenvolvimento de Sistemas (ADS) da **CESAR School** (Turma ADS20262_2A - Semestre 2026.2).

## 👨‍🏫 Docente
**Prof. Danilo Farias Soares da Silva**

## 📖 Ementa
Este repositório explora a introdução a dois dos principais paradigmas de linguagem de programação: **imperativa** e **funcional**.

* **Linguagem C (Imperativa):** comandos de entrada e saída; estruturas de decisão e repetição; matrizes, strings e tipos de dados estruturados; subprogramas, arquivos, conjuntos, ponteiros; alocação dinâmica de memória; introdução à programação modular.
* **Linguagem Haskell (Funcional):** listas, tuplas, tipos, classes, polimorfismo, funções de alta ordem, recursão.

## 🎯 Objetivos
* Dominar os conceitos básicos de programação imperativa e o gerenciamento de memória (ponteiros e alocação dinâmica) usando C.
* Compreender os princípios fundamentais da programação funcional, desenvolvendo a capacidade de resolver problemas de forma declarativa em Haskell.
* Entender as características, vantagens e desvantagens de cada paradigma na resolução de problemas computacionais.

## 📅 Estrutura do Repositório (Cronograma)

### 📁 Unidade 1: Programação Imperativa (C)
- `01_introducao/`: Ambiente de desenvolvimento e "Hello World".
- `02_decisao_repeticao/`: `if/else`, `switch`, `while`, `for`.
- `03_estruturas_dados/`: Vetores, matrizes e strings.
- `04_funcoes/`: Modularização, escopo e passagem de parâmetros.
- `05_ponteiros_memoria/`: Aritmética de ponteiros e alocação dinâmica (`malloc`, `free`).

### 📁 Unidade 2: Programação Funcional (Haskell)
- `06_intro_haskell/`: GHCi, listas e tuplas.
- `07_tipos_classes/`: Sistema de tipos em Haskell.
- `08_polimorfismo/`: Funções polimórficas (ex: função `id`).
- `09_alta_ordem/`: Funções de alta ordem (`map`, `fold`).
- `10_recursao_adts/`: Funções recursivas e Tipos de Dados Algébricos.

## ⚙️ Ferramentas Utilizadas
- **C:** Compilador GCC, IDE recomendada (VS Code, CLion, etc.)
- **Haskell:** GHC (Glasgow Haskell Compiler) e GHCi (REPL)
- **Ambiente:** AWS Academy

## 📚 Bibliografia Principal
* SCHILDT, H. *C Completo e Total*.
* THOMPSON, S. *Haskell: The Craft Of Functional Programming*.

---
💡 *Dica: Navegue pelas pastas para encontrar os códigos de exemplo apresentados em aula e as resoluções das Listas de Exercícios.*
